...
class AbstractXmlLLMReplacement(Edit):
    NODE_TAG = ''
    @classmethod
    def auto_create(cls,ref):
        target = ref.random_model(AbstractXmlModel).random_target(cls.NODE_TAG)
    ...
    def apply(self,ref,variant):
        model = variant.models[self.target[0]]
        return model.do_llm_replace(self.target)

class AbstractXmlBooleanReplacement(Edit):
    NODE_TAG = ''
    @classmethod
    def auto_create(cls,ref):
        target = ref.random_model(AbstractXmlModel).random_target(cls.NODE_TAG)
    ...
    def apply(self,ref,variant):
        model = variant.models[self.target[0]]
        return model.do_boolean_replace(self.target)
