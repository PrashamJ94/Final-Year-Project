...
class XmlModel(AbstractXmlModel):
    ...
    def init_contents(self):
        ...
        # for child in tree:
        #     print(child.tag)
        #     print(ElementTree.tostring(child, encoding='unicode'))
    ...
    def do_boolean_replace(self, target):
        d_f, d_t, d_i = target
        ...
        code = target.text
        # find if the code contains true or false
        new_code = code
        if 'False' in code:
            new_code = code.replace('False', 'True')
        elif 'True' in code:
            new_code = code.replace('True', 'False')
        elif 'false' in code:
            new_code = code.replace('false', 'true')
        elif 'true' in code:
            new_code = code.replace('true', 'false')
        
        target.text = new_code
        return True
    ...
    def do_llm_replace(self, target):
        d_f, d_t, d_i = target
        ...
        # give the LLM the text of the node and ask it to mutate the type
        code = target.text
        new_code = self.get_llm_type_transform(code)
        
        print(code, new_code)
        # mutate
        target.text = new_code
        if 'long a =' in new_code:
            print(target.text)
            print(target.tag)
            print(target.attrib)
    ...
    def get_llm_type_transform(self, code):
        prompt = """Here is some expression from a programming language: {code} 
        You are tasked with modifying the datatype in the expression if any. If no datatype is present in the expression, return the same expression.
        
        Here's an example of how you might approach the task when given an expression of the form "int x = 5;":
        1. Identify if any datatype is in the expression, here "int" is the datatype.
        2. Think of which programming language it might belong to, here it is Java.
        3. Modify the datatype in the expression. For example "int x = 5;" can be modified to "long x = 5;".
        4. Return only the modified expression like "long x = 5;".
        5. If no datatype is present in the expression, return the same expression "int x = 5;".
        
        Don't return any additional text, only the modified or unmodified expression. Here is the expression again: {code} Your output response must adhere to the text format: Expression: ***the expression***. Your output response should
        not contain any text on thinking.
        """.format(code=code)
