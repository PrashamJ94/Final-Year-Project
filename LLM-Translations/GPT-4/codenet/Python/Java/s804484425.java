...
public class s804484425 {
    public static void main(String[] args) {
        ...
        long sumSelect = selectedChohuku.stream().mapToInt(Integer::intValue).sum() + selectedOnly.stream().mapToInt(Integer::intValue).sum();
        long ans = cntType * cntType + sumSelect;
        ...
        for (int i = 0; i < Math.min(selectedChohuku.size(), unselectedOnly.size()); i++) {
            ...
            long tmp = cntType * cntType + sumSelect;
